...
Register also returns `PasskeyCredentialResponse` (`credentialId`; `displayName` and `createdAt` when present). Completers only establish a session when `CanSignInAsync` succeeds (same confirmed-account / lockout policy as Identity login and OAuth).
