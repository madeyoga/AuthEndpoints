---
title: Configuration
description: Reference for AuthEndpointsOptions and nested passkey, JWT, ReAuth, and email confirmation settings.
navigation:
...
| `ReAuth` | 5 minutes | Nested [ReAuth](#reauth) cookie and `reauthToken` lifetime |
...
SPA signup that stays logged out until confirm: [Register a confirmed account](/examples/register-confirmed-account).
...
`AuthEndpointsReAuthOptions` (`o.ReAuth`):

| Property | Default | Description |
| --- | --- | --- |
| `Lifetime` | 5 minutes | Cookie `ExpireTimeSpan`, ticket `ExpiresUtc`, and header `reauthToken` expiry for the same confirm. Non-persistent (`IsPersistent = false`). No sliding expiration. |

Startup validation rejects `<= 0`, `InfiniteTimeSpan`, values under 1 minute, and values over 60 minutes.

Composable hosts that skip the facade configure the same type:

```cs
builder.Services.Configure<AuthEndpointsReAuthOptions>(o =>
{
