...
JWT is opt-in (`o.Jwt.Enabled = true`). Identity bearer uses the same facade with `AuthEndpointsSignIn.IdentityBearer`, or compose (`AddBearerAuthEndpoints` / `MapBearerAuthEndpoints`).
...
app.MapAuthEndpoints<AppUser>(); // /identity (management + bearer) + /account (passkeys)
