...
---
id: cCTn7nZz3wI
title: Register and account activation
---
::
...
2. Call `GET /identity/csrfToken` before POSTs that require antiforgery (passkey ceremonies, logout, manage, ReAuth). Password `POST /identity/register` and `POST /identity/login` do not require it. Send the value as header `RequestVerificationToken`.
...
2. `POST /identity/register` with body `{ "email", "password" }`. CSRF is not required on register.
