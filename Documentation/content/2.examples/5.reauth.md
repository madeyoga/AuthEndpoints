...
Sensitive manage mutations and any host endpoint with `.RequireReauth()` need a fresh ReAuth proof. Browser clients get the `AuthEndpoints.ReAuth` cookie (default 5 minutes, `ReAuth.Lifetime`). API clients can send header `X-AuthEndpoints-Reauth` with the `reauthToken` from confirm. Cookie and token share that lifetime.
